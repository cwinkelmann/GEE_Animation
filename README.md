...
- **`viz`** (optional; min/max/palette): fixed range for colorization so colour is comparable across frames. If omitted, per-index defaults apply (NDVI −0.2..0.9 green; EVI 0..1 green; NDWI −0.3..0.6 brown→blue; NDMI −0.5..0.8 brown→teal; LST 0..40°C thermal). For **LST** a single fixed range can't fit both a cool spring and a hot summer, so a mid-range default washes out any one month — set `viz` to your season (the thermal example configs use 10..40 °C), or use anomaly rendering (planned; renders each pixel against its own long-term monthly value) which adapts automatically.
