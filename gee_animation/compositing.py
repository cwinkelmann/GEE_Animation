...
# "quarterly" is not in this table: its periods span three whole months, so it is
# generated by its own branch in `period_starts` (and anomaly rejects it, since a
# quarter mean can't be scored against a monthly climatology either).
_SPLIT_DAYS = {
    "monthly": (1,),
    "semimonthly": (1, 16),
    "10day": (1, 11, 21),
...
def period_starts(start: str, end: str, cadence: str) -> list[tuple[str, str, str]]:
    ...
    quarterly labels are "YYYY-Qn" (calendar quarters: Q1 = Jan–Mar).
    """
    if cadence == "quarterly":
        # Whole calendar quarters, aligned to Jan/Apr/Jul/Oct regardless of where
        # `start` falls — a run starting mid-quarter still composites the full
        # quarter that contains it, the same way a monthly run starting on the
        # 15th composites the whole month.
        month = date.fromisoformat(start).replace(day=1)
        month = month.replace(month=(month.month - 1) // 3 * 3 + 1)
        stop = date.fromisoformat(end)
        out = []
        while month < stop:
            nxt = _add_month(_add_month(_add_month(month)))
            label = f"{month.year}-Q{(month.month - 1) // 3 + 1}"
            out.append((label, month.isoformat(), nxt.isoformat()))
            month = nxt
        return out
...
def _calendar_key(p_start_iso: str, p_end_iso: str) -> tuple[tuple[int, ...], int, int]:
    """(months, first_day, last_day_inclusive) — a period's calendar signature with
    the year stripped, i.e. the slot it occupies in *every* year.

    Monthly and sub-monthly periods never span two calendar months (see
    :data:`_SPLIT_DAYS`), so for them `months` is a single-element tuple and the
    day-of-month range identifies the slot within it. A quarterly period covers
    three whole months, so `months` lists all three and the day range is the whole
    month (1–31) — matching by month membership is what keeps pooling from
    silently considering only the quarter's first month. The last day is 31 for a
    period that runs to the end of its month, so February's shorter months still
    match — the range is a filter, not a calendar.
    """
    p_start, p_end = date.fromisoformat(p_start_iso), date.fromisoformat(p_end_iso)
    # p_end is exclusive; the last month the period touches is the month before it
    # when the period ends on a month boundary.
    last = p_end if p_end.day > 1 else date(
        p_end.year - (p_end.month == 1), (p_end.month - 2) % 12 + 1, 1)
    if (last.year, last.month) == (p_start.year, p_start.month):
        last_day = 31 if p_end.day == 1 else p_end.day - 1
        return (p_start.month,), p_start.day, last_day
    months = []
    m = p_start
    while (m.year, m.month) <= (last.year, last.month):
...
def pooled_composite(collection, cfg, ee_module=ee) -> list[Frame]:
    ...
    * ``gap_fill`` — **true gap filling, and the only strategy that preserves the
    ...
    for label, p_start, p_end in periods:
        months, first_day, last_day = _calendar_key(p_start, p_end)
        candidates = [i for i, d in enumerate(dates)
                      if y0 <= d.year <= y1 and d.month in months
